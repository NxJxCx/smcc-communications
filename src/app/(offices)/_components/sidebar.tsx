...
export default function SidebarComponent() {
  ...
  const role = useMemo(() => status === "authenticated" ? session?.user?.role || undefined : undefined, [session?.user?.role, status])
  ...
  return (
    <aside className={clsx(
      "float-left min-w-[220px] max-w-[220px] p-2 bg-[#004aad] shadow flex-shrink text-white px-6",
        ...
        {status === 'authenticated' && (
          <>
            ...
            <div className="w-full flex flex-col justify-start items-center gap-y-4 font-[600] flex-shrink py-8">
              <button onClick={onLogout} className="w-full rounded-full text-left hover:border hover:border-yellow-500 pl-4 py-1">
