...
export default function AddFacultyAccountModal({
  open,
  onClose = () => {},
  onRefresh = () => {}
}: Readonly<{
  ...
  const [state, action, pending] = useFormState<ActionResponseType, FormData>(addUserAccount.bind(null, Roles.Faculty), {})
  ...
  return (
    <OCSModal title="Add Faculty Account" open={open} onClose={onClose}>
      <div className="p-4">
        <form action={action} ref={formRef} className="flex flex-col gap-y-2">
          <input type="text" name="employeeId" placeholder="Employee ID" value={employeeId} onChange={(e) => setEmployeeId(e.target.value)} className="px-2 py-1 rounded bg-white border border-slate-400" />
