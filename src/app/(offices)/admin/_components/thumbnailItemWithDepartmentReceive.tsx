...
export default function ThumbnailItemWithDepartmentReceive({ layout, thumbnailSrc, department, label, createdAt, updatedAt, isRejected, preparedByMe, isPending, onClick, isRead }: { layout: ViewLayout, thumbnailSrc: string; department: string; label: string|React.ReactNode; createdAt?: Date|string|null, updatedAt?: Date|string|null, isRejected?: boolean, preparedByMe?: boolean, isPending?: boolean, onClick: (e?: any) => void, isRead?: boolean }) {
  return (<>
    ...
    {layout === "list" && (
          <button type="button" onClick={onClick} className={clsx("hover:bg-gray-400/10 px-3 py-1 rounded-lg grid grid-cols-5 text-left whitespace-nowrap items-center w-full", isRejected ? "bg-red-300 hover:bg-red-500" : preparedByMe ? "bg-sky-100 hover:bg-sky-300" : isPending ? "bg-yellow-100 hover:bg-yellow-300" : "bg-white")}>
